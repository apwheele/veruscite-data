...
Citation extraction uses an LLM to parse the document text and identify individual references. The table below shows extraction accuracy **and wall-clock speed** against the ground truth (latest kept runs, 2026-07-22).
...
Extraction is very accurate across models. **Gemini 3.1 Flash Lite is the default production extraction model**: it correctly extracts about **`{python} f"{default_rate:.2f}"`%** of citations (highest correct count, fewest missing/extra among the kept runs) and finishes the full 36-document corpus in about **`{python} f"{default_wall_min:.1f}"` minutes** wall time (~`{python} f"{default_dpm:.1f}"` docs/min), roughly twice as fast as `gpt-5.4-nano` on the same batch. Cost remains on the order of a few cents per paper for the full corpus.

The few "extra" rows that remain are typically table fragments or rare non-bibliography lines the model still surfaces; numbered discursive footnotes and author bios are filtered in the current pipeline.

`gpt-5.4-nano` is retained as a comparison baseline. Both runs use `pypdfium2` for PDF text extraction.
...
- `extraction_run/` -- raw extraction outputs per model (kept: Gemini 3.1 Flash Lite + gpt-5.4-nano, 2026-07-22)
...
This paper was prepared with AI assistance. Drafting and earlier iterations used **Claude Opus 4.6** (Anthropic), reviewing prior works by Andrew Wheeler (see @wheeler2026claude for that workflow). Updates for the 2026-07-22 extraction default (Gemini 3.1 Flash Lite), speed metrics, and this disclosure were assisted by **Grok 4.5** (xAI). Ground-truth labels, model-default decisions, and final review are human.
